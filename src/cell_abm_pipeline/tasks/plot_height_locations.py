...
@task
def plot_height_locations(keys, data, tick=0, reference=None, region=None, padding=50):
    ...
    if reference is not None:
        reference_height = reference[value].mean()
    else:
        reference_height = all_data[all_data["TICK"] == tick][value].mean()
    ...
    for i, j, (key, seed) in indices:
        ...
        heights = (tick_data[value] - reference_height) / reference_height

        sax = ax.scatter(x, y, c=heights, s=20, cmap="coolwarm", vmin=-1, vmax=1)
