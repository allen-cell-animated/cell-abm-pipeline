...
class pyMCDS:
    ...
    def get_concentrations(self, species_name, z_slice=None):
        ...
        if z_slice is not None:
            ...
            zz = self.data["mesh"]["z_coordinates"]
            assert z_slice in zz, "Specified z_slice {} not in z_coordinates".format(
                z_slice
            )
    ...
    def get_cell_df_at(self, x, y, z):
        ...
        inside_voxel = (
            (cell_df["position_x"] < x_vox + ds / 2.0)
            & (cell_df["position_x"] > x_vox - ds / 2.0)
            & (cell_df["position_y"] < y_vox + ds / 2.0)
            & (cell_df["position_y"] > y_vox - ds / 2.0)
            & (cell_df["position_z"] < z_vox + ds / 2.0)
            & (cell_df["position_z"] > z_vox - ds / 2.0)
        )
